...
contract ERC721AUpgradeableInternal is IERC721AUpgradeable {
    ...
    /// @notice Mask of an entry in packed address data
    uint256 internal constant _BITMASK_ADDRESS_DATA_ENTRY = (1 << 64) - 1;

    /// @notice The bit position of `numberMinted` in packed address data
    uint256 internal constant _BITPOS_NUMBER_MINTED = 64;

    /// @notice The bit position of `numberBurned` in packed address data
    uint256 internal constant _BITPOS_NUMBER_BURNED = 128;

    /// @notice The bit position of `aux` in packed address data
    uint256 internal constant _BITPOS_AUX = 192;

    /// @notice Mask of all 256 bits in packed address data except the 64 bits for `aux`
    uint256 internal constant _BITMASK_AUX_COMPLEMENT = (1 << 192) - 1;

    /// @notice The bit position of `startTimestamp` in packed ownership
    uint256 internal constant _BITPOS_START_TIMESTAMP = 160;

    /// @notice The bit mask of the `burned` bit in packed ownership
    uint256 internal constant _BITMASK_BURNED = 1 << 224;

    /// @notice The bit position of `nextInitialized` in packed ownership
    uint256 internal constant _BITPOS_NEXT_INITIALIZED = 225;

    /// @notice The bit mask of the `nextInitialized` bit in packed ownership
    uint256 internal constant _BITMASK_NEXT_INITIALIZED = 1 << 225;

    /// @notice The bit position of `extraData` in packed ownership
    uint256 internal constant _BITPOS_EXTRA_DATA = 232;

    /// @notice Mask of all 256 bits in a packed ownership except the 24 bits for `extraData`
    uint256 internal constant _BITMASK_EXTRA_DATA_COMPLEMENT = (1 << 232) - 1;

    /// @notice The mask of the lower 160 bits for addresses
    uint256 internal constant _BITMASK_ADDRESS = (1 << 160) - 1;

    /**
     * @notice The maximum `quantity` that can be minted with {_mintERC2309}.
    ...
    /// @notice The `Transfer` event signature
    ...
    /**
     * @dev Returns the auxiliary data for `owner`. (e.g number of whitelist mint slots used)
     */
    ...
    /**
     * @dev Sets the auxiliary data for `owner`. (e.g. number of whitelist mint slots used)
     ...
     * it can be overridden in child contracts.
     */
     ...
     * It gradually moves to 0(1) as tokens get transferred around over time
     */
     ...
     * An unintialized slot does not necessarily mean that the slot has no owner.
     */
     ...
     */
    function _initialOwnershipAt(uint256 index) internal virtual {
    ...
    /**
     * @dev Returns the `nextInitialized` flag set if `quantity` equals to 1.
     */
    function _nextInitializedFlag(uint256 quantity) internal pure returns (uint256 result) {
        // For branches setting of the `nextInitialized` flag
        assembly {
