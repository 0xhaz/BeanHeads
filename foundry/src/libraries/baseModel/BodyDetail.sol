...
library BodyDetail {
    ...
    function onlyBreastSVG(uint8 color, uint8 skinColor) internal pure returns (string memory) {
        ...
        return SVGBody.fullSVG(
            ...
            string(
                abi.encodePacked(
                    ...
                    '<path d="M494.47,852.86a37.23,37.23,0,0,1,6,12.13,48.61,48.61,0,0,1,.15,27,37.23,37.23,0,0,1-5.93,12.19c.43-4.53,1-8.8,1.29-13.05s.5-8.43.48-12.63-.22-8.39-.6-12.63S495,857.38,494.47,852.86Z" style="fill:',
                    BytesConverter.bytesToHex(shadowColor),
                    '"/>',
                    '<path d="M505.76,852.29c-.93,5.5-1.74,10.77-2.1,16a103.31,103.31,0,0,0,2.43,31.17c1.19,5.15,2.8,10.23,4.57,15.53A44.76,44.76,0,0,1,501.6,901,55,55,0,0,1,499,867.6,44.61,44.61,0,0,1,505.76,852.29Z" style="fill:',
                    BytesConverter.bytesToHex(shadowColor),
                    '"/>',
                    ...
                    BytesConverter.bytesToHex(shadowColor),
                    '"/>',
                    '<path d="M502.07,878.86A69.31,69.31,0,1,1,371,847.39" fill="none" stroke=',
                    BytesConverter.bytesToHex(shadowColor),
                    ' strokeMiterlimit={10} strokeWidth="12px"/>',
                    '<path d="M371,847.39a69.68,69.68,0,0,1,15-19.67" fill=',
                    BytesConverter.bytesToHex(skinBaseColor),
                    " />",
                    '<path d="M365.65,844.66a27,27,0,0,1,3.38-5.84,37.22,37.22,0,0,1,4.53-5c.84-.76,1.69-1.5,2.58-2.19a21.1,21.1,0,0,1,2.81-1.92,15,15,0,0,1,7-2,25,25,0,0,1-1.75,6.81c-.39,1-.83,1.88-1.29,2.77l-1.3,2.67c-.85,1.76-1.69,3.49-2.55,5.19a54.51,54.51,0,0,1-2.76,5Z" fill=',
                    BytesConverter.bytesToHex(shadowColor),
                    " />",
                    '<path d="M489.33,838.82a69,69,0,0,1,12.74,40" fill=',
                    BytesConverter.bytesToHex(skinBaseColor),
                    " />",
                    ...
                    '<path d="M641.65,878.86a69.14,69.14,0,0,0-21.94-50.6" fill=',
                    BytesConverter.bytesToHex(skinBaseColor),
                    " />",
                    '<path d="M635.65,878.86A107,107,0,0,0,634.39,866a81.92,81.92,0,0,0-3.12-12.67c-.34-1-.74-2.07-1.11-3.11s-.84-2.05-1.25-3.09-.94-2-1.38-3.05l-1.46-3c-2-4.1-4.17-8.19-6.36-12.75a35.76,35.76,0,0,1,12.22,8.61c.83.94,1.7,1.86,2.47,2.85s1.54,2,2.25,3,1.43,2.09,2.07,3.18,1.31,2.18,1.88,3.32A62.19,62.19,0,0,1,646,863.58a56.6,56.6,0,0,1,1.62,15.28Z" fill=',
                    BytesConverter.bytesToHex(shadowColor),
                    " />",
                    '<path d="M503,878.86a69.31,69.31,0,1,0,138.61,0" fill="none" stroke=',
                    BytesConverter.bytesToHex(shadowColor),
                    ' strokeMiterlimit={10} strokeWidth="12px" />',
                    '<path d="M515.77,838.82a69,69,0,0,0-12.73,40" fill=',
                    BytesConverter.bytesToHex(skinBaseColor),
                    " />",
                    '<path d="M515.77,838.82a104.17,104.17,0,0,1-2.27,10.75c-.42,1.67-.87,3.3-1.29,4.93l-.57,2.45-.5,2.44c-.63,3.26-1.09,6.5-1.41,9.74s-.5,6.48-.69,9.73H497a47,47,0,0,1,.94-11.53,54.48,54.48,0,0,1,3.46-11c.77-1.76,1.61-3.48,2.54-5.15a40,40,0,0,1,3.1-4.79A26.66,26.66,0,0,1,515.77,838.82Z" fill=',
                    BytesConverter.bytesToHex(shadowColor),
                    " />",
    ...
    function getColorsForBody(uint8 id) internal pure returns (bytes3 baseColor, bytes3 shadowColor) {
        if (id == uint8(BodyColor.LIGHT_SKIN)) {
            return (Colors.LIGHT_SKIN_BASE, Colors.LIGHT_SKIN_SHADOW);
        } else if (id == uint8(BodyColor.YELLOW_SKIN)) {
            return (Colors.YELLOW_SKIN_BASE, Colors.YELLOW_SKIN_SHADOW);
        } else if (id == uint8(BodyColor.BROWN_SKIN)) {
            return (Colors.BROWN_SKIN_BASE, Colors.BROWN_SKIN_SHADOW);
        } else if (id == uint8(BodyColor.DARK_SKIN)) {
            return (Colors.DARK_SKIN_BASE, Colors.DARK_SKIN_SHADOW);
        } else if (id == uint8(BodyColor.RED_SKIN)) {
            return (Colors.RED_SKIN_BASE, Colors.RED_SKIN_SHADOW);
        } else if (id == uint8(BodyColor.BLACK_SKIN)) {
            return (Colors.BLACK_SKIN_BASE, Colors.BLACK_SKIN_SHADOW);
        } else {
            revert Errors.InvalidColor(id);
        }
    }
