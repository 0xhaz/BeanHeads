...
library BodyDetail {
    ...
    function breastSVG(uint8 clothingColor, uint8 skinColor) internal pure returns (string memory) {
    ...
    /// @param clothingColor The breast color id
    /// @return Combined SVG for chest and breast
    function womanBodySVG(uint8 chestId, uint8 clothingColor) internal pure returns (string memory) {
        string memory chest = chestSVG(chestId);
        string memory breast = breastSVG(clothingColor, chestId);
